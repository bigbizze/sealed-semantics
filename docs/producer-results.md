...
The completed kind returns the producer's failure unchanged. It does not wrap errors, add properties, or convert them into messages. Object unions, string errors, and other producer-owned values are supported. Successful mint calls snapshot Parts and create distinct instances. If a success contains invalid Parts, `.mint()` throws a misuse error instead of returning a failed producer result. Minted values have no serialization API in the current API.
