...
`defineKind(...)` or `defineDerived(...)` starts a builder. Add optional `.view(...)` projections, then optional `.docs(...)` metadata, and finish with `.seal()`. Sealing checks the metadata, executes semantic examples, and returns a frozen kind. Invalid examples throw synchronously at this step, including during module initialization. Derived documentation checks do not call `derive`. Configure views before documentation so its types describe the final projections.
...
| Projection `description` | Required string; `.seal()` rejects blank descriptions. |
...
Read the metadata through `Kind.documentation`. Its containers, examples array, example records, view map, and projection-description records are copied and frozen. The sample graphs themselves remain author-owned references and are not cloned or frozen. Calling `.docs(...)` again before sealing replaces the whole document in a new builder. Completed kinds expose `documentation`, but no configuration methods. Unsupported capabilities are absent from autocomplete; generic checks diagnose extra supplied keys. Keys erased by a wider TypeScript annotation cannot be checked statically.

## Validation at definition time

`.seal()` validates every linked example through the real Zod codec and `Kind.parse`, checks their brands and equality, and compares actual encoding and canonical output with the documented outputs. It reparses encoded output and checks stable encoding and equality. No CLI, module discovery, test-only docs import, or separate CI command is required. A test that imports the definition executes the check; compilation alone does not. A kind without `.docs()` remains valid.
...
Validation runs when a builder is first sealed. Repeated `.seal()` calls on that builder return the same kind without rerunning its examples. It does not change subsequent parsing or encoding. Producers must therefore permit their ordinary callbacks to run during module initialization. The guarantee applies at validation time; sample graphs remain author-owned and must not be changed if the documentation is to remain accurate.
...
For example, if `input` is valid but the documented canonical value has a typo, `.seal()` throws with the kind, example index, and canonical mismatch. If `input` itself is invalid, that rejection occurs first. The examples are linked assertions about actual transformations, not just values that happen to have compatible TypeScript types.
