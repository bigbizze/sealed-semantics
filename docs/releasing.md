...
4. Inspect `npm pack --dry-run`. `npm pack` runs the checks through prepack and creates `sealed-semantics-0.3.3.tgz` for the current version.
5. Share the tarball directly with `npm install /path/to/sealed-semantics-0.3.3.tgz`, or publish with `npm publish --access public` from an authorized npm account after choosing the final version and package name.
