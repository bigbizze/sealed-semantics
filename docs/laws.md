...
The harness is test-only and traverses projections, never private Parts. Built-in mutators accept frozen plain objects and arrays, and still visit their children. Function-valued projections require an explicit projectionMutator. Snapshots inspect own data properties without invoking accessors; closure state and accessor results need separate domain observations. Custom types with private state need a mutator that makes an observable change; the observer captures enumerable and non-enumerable own state plus standard Date, Map, Set, and buffer content, but cannot inspect arbitrary private class state. Test such projections through additional domain observations. A dishonest or ineffective mutator cannot establish alias safety.
