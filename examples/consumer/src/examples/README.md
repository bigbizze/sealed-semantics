...
The array is copied during preparation. `view.plans` caches and deeply freezes it. The sealed child values can be shared. Authorization, transactions, and database constraints remain responsibilities of the application and adapter.

## Decode an HTTP request and encode a response

[http-contract.ts](define-kind/http-contract.ts) provides a fake `POST /memberships/preview` handler. It reads JSON, validates IDs, calls a function that accepts the specific sealed types, and returns encoded JSON.
...
Section 2 shows that equivalent decodes return the same live object. It also encodes the full schema back to raw data. Section 3 compares ordinary parse results with exceptions and HTTP errors.

## Reuse a cached profile

[profile-cache.ts](define-kind/profile-cache.ts) accepts a database lookup function and returns a cached lookup. A native `Map<UserId, Profile>` treats a legacy spelling and the current spelling as the same key after normalization. Section 4 makes four sequential lookups for two users and logs two database reads.
