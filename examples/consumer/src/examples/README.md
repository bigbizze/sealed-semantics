...
Read [membership-workflow.ts](define-derived/membership-workflow/membership-workflow.ts), starting with `saveMembershipBatch`. Its first argument must be a `MembershipBatch`; a matching object shape is insufficient.
...
2. `PreparedMembership.derive({ userId, projectId })` requires both IDs and checks their runtime brands.
3. [MembershipBatch.derive](define-derived/membership-workflow/membership-batch.ts) accepts the plans, rejects an empty list or mixed projects, and keeps the first plan for each user.
...
Section 1 uses both `z.safeDecode(postResponseSchema, responseBody)` and individual `parseOrThrow()` calls. A schema is usually more convenient for complete requests and responses: it validates the structure and decodes nested sealed values in one operation. Individual parsing is useful when only one property needs decoding.
